...
import { Plus, Search, FileText, Loader2, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react';
...
import type { Lease, LeaseStatus, PaymentFrequency, PaginationMeta } from '@/types';
...
const FREQ_LABELS: Record<PaymentFrequency, string> = {
  monthly: 'Mensuel', 'bi-weekly': 'Bimensuel', weekly: 'Hebdo',
...
function LeaseRow({ lease, selected, onClick }: {
  lease: Lease; selected: boolean; onClick: () => void;
}) {
  ...
  const nearExpiry = lease.status === 'active' && daysLeft <= 30 && daysLeft >= 0;
  ...
  return (
    ...
    >
      <td className="px-5 py-3.5">
        <p className="text-[13px] font-medium text-primary truncate">{tenantName}</p>
      ...
      <td className="px-4 py-3.5 text-[12px] text-primary/50">{FREQ_LABELS[lease.paymentFrequency]}</td>
      <td className="px-4 py-3.5 text-[12px] text-primary/50 tabular-nums whitespace-nowrap">
        {formatDate(lease.startDate)}
      </td>
      <td className="px-4 py-3.5 whitespace-nowrap">
        <p className={`text-[12px] tabular-nums ${nearExpiry ? 'text-secondary font-semibold' : 'text-primary/50'}`}>
...
function PaginationBar({ meta, onPage }: { meta: PaginationMeta; onPage: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center justify-between px-5 py-3 border-t border-[#E5E7EB] bg-surface shrink-0">
      <p className="text-[12px] text-primary/40 tabular-nums">{from}–{to} sur {total} contrat{total > 1 ? 's' : ''}</p>
      <div className="flex items-center gap-1">
        ...
        <span className="px-3 text-[13px] font-medium text-primary tabular-nums">{page} / {totalPages}</span>
    ...
    <>
      <div className="flex h-screen overflow-hidden">
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
          ...
          {/* Toolbar */}
          <div className="flex items-center justify-between px-6 py-4 bg-surface border-b border-[#E5E7EB] shrink-0">
            <div>
              <h1 className="font-semibold text-[20px] text-primary">Contrats de bail</h1>
              {pagination && !loading && (
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input type="search" value={search} onChange={(e) => setSearch(e.target.value)}
                  ...
                  className="pl-9 pr-4 h-9 w-48 rounded-lg border border-[#E5E7EB] bg-white text-[13px]
          ...
          {/* Status filter pills */}
          <div className="flex items-center gap-1.5 px-6 py-2.5 border-b border-[#E5E7EB] bg-surface shrink-0 overflow-x-auto">
          ...
          {error && (
            <div className="mx-6 mt-4 flex items-center gap-2 px-4 py-3 rounded-lg bg-danger/8
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 z-10 bg-neutral">
                  <tr className="border-b border-[#E5E7EB]">
                    {['Locataire / Local', 'Loyer', 'Fréquence', 'Début', 'Fin', 'Statut'].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-primary/40">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#E5E7EB] bg-surface">
                  {leases.map((l) => (
                    <LeaseRow key={l.id} lease={l} selected={selected?.id === l.id}
          ...
          {pagination && pagination.totalPages > 1 && <PaginationBar meta={pagination} onPage={setPage} />}
        </div>
      ...
      <LeaseFormModal
        isOpen={formOpen} lease={editTarget}
      ...
      {terminateTarget && (
        <LeaseTerminateModal
          isOpen={!!terminateTarget} lease={terminateTarget}
