...
import { Plus, Search, Building2, Loader2, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const TYPE_LABELS: Record<string, string> = {
  Apartment: 'Appartement', House: 'Maison', Commercial: 'Commercial',
  Office: 'Bureau', Warehouse: 'Entrepôt', Other: 'Autre',
};
...
function PropertyRow({
  property, selected, onClick,
}: { property: Property; selected: boolean; onClick: () => void }) {
  return (
    <tr
      onClick={onClick}
    ...
    >
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-primary/6 flex items-center justify-center shrink-0">
            <Building2 size={14} className="text-primary/50" aria-hidden="true" />
          </div>
          <div className="min-w-0">
            <p className="text-[13px] font-medium text-primary truncate">{property.name}</p>
      ...
      <td className="px-4 py-3.5 text-[13px] text-primary/50 max-w-[200px] truncate">
        {property.address}
      </td>
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="w-8 h-8 rounded-lg flex items-center justify-center
                     text-primary/50 hover:text-primary hover:bg-primary/6
    ...
    <>
      <div className="flex h-screen overflow-hidden">
        ...
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-surface border-b border-[#E5E7EB] shrink-0">
            <div>
              <h1 className="font-semibold text-[20px] text-primary">Biens immobiliers</h1>
              {pagination && !loading && (
                <p className="text-[12px] text-primary/40 mt-0.5">
                  {pagination.total} bien{pagination.total > 1 ? 's' : ''} enregistré{pagination.total > 1 ? 's' : ''}
                </p>
              )}
            </div>
            <div className="flex items-center gap-3">
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-primary/35 pointer-events-none" aria-hidden="true" />
                <input
                  type="search"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Rechercher…"
                  className="pl-9 pr-4 h-9 w-56 rounded-lg border border-[#E5E7EB] bg-white
                             text-[13px] text-primary placeholder:text-primary/30
              ...
              <button
                ...
                className="flex items-center gap-2 h-9 px-4 bg-primary text-white
          ...
          {error && (
            <div className="mx-6 mt-4 flex items-center gap-2 px-4 py-3 rounded-lg
                            bg-danger/8 border border-danger/20 text-[13px] text-danger shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center h-48">
                <Loader2 size={22} className="animate-spin text-primary/30" aria-hidden="true" />
              </div>
            ) : properties.length === 0 ? (
              <div className="p-6">
                <EmptyState
                  ...
                  onAction={debouncedQ ? undefined : () => { setEditTarget(null); setFormOpen(true); }}
                />
              </div>
            ) : (
              <table className="w-full border-collapse">
                <thead className="sticky top-0 z-10 bg-neutral">
                  <tr className="border-b border-[#E5E7EB]">
                    {['Bien', 'Type', 'Locaux', 'Adresse', 'Créé le'].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-[11px] font-medium uppercase tracking-[0.06em] text-primary/40">
                        {h}
                      </th>
                    ))}
                  </tr>
          ...
          <div className="flex items-center justify-end gap-3">
            <button
              ...
              className="h-10 px-5 rounded-lg text-[14px] font-medium text-primary/60
                         hover:text-primary border border-[#E5E7EB] hover:border-primary/30
                         transition-colors duration-150"
            >
              Annuler
            </button>
      ...
      >
        <div className="space-y-3">
          ...
          {deleteError && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg bg-danger/8
