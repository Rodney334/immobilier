...
export function LeaseTerminateModal({ lease, isOpen, onClose, onDone }: Props) {
  const [state, formAction] = useActionState(
    async (_prev: FormState, formData: FormData): Promise<FormState> => {
      ...
      if (!terminationDate) {
        return {
          error: "La date de résiliation est obligatoire.",
          success: false,
        };
      }

      try {
        const res = await leaseService.terminate(lease.id, {
          terminationDate,
      ...
      <form id="terminate-form" action={formAction} className="space-y-4">
        <div className="px-4 py-3 rounded-lg bg-danger/5 border border-danger/15">
          <p className="text-[13px] text-danger leading-relaxed">
            ...
            <span className="font-semibold">{tenantName}</span>. Cette action
            est irréversible et clôturera les échéances futures.
          </p>
        </div>
